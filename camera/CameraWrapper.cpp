...
camera_module_t HAL_MODULE_INFO_SYM = {
    common: {
         ...
         name: "Tegra 3 Camera Wrapper for tf300t",
...
char * camera_fixup_setparams(int, const char * settings)
{
    android::CameraParameters params;
    params.unflatten(android::String8(settings));

    // tf300t blobs don't know auto antibanding so set something else
    if (params.get("antibanding")) {
        const char* antibanding = params.get(android::CameraParameters::KEY_ANTIBANDING);
	if (strcmp(antibanding, "auto") == 0)
	  params.set(android::CameraParameters::KEY_ANTIBANDING, "60hz");
    }
...
static void camera_put_custom_parameters(struct camera_device *device, char *params)
{
    ALOGV("%s", __FUNCTION__);
    ALOGV("%s->%08X->%08X", __FUNCTION__, (uintptr_t)device, (uintptr_t)(((wrapper_camera_device_t*)device)->vendor));
    ...
    VENDOR_CALL(device, put_parameters, tmp);
    if(params)
        free(params);
}
                ...
                hw_device_t** device)
{
    ...
    if (name != NULL) {
        ...
        camera_ops->put_parameters = camera_put_custom_parameters;
